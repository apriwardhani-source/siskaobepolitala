...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-6 px-4 sm:px-6 lg:px-8">
    <div class="max-w-5xl mx-auto">
        ...
        <!-- Header -->
        <div class="mb-6">
            <a href="{{ route('tim.capaianpembelajaranlulusan.index', ['id_tahun' => $id_cpl->id_tahun]) }}" 
            ...
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight">Edit Capaian Profil Lulusan (CPL)</h1>
                    <p class="mt-1 text-sm text-gray-600">
                        Perbarui informasi CPL untuk program studi Anda pada tahun kurikulum terkait.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
            ...
            <div class="px-6 py-6">
                ...
                <form action="{{ route('tim.capaianpembelajaranlulusan.update', $id_cpl->id_cpl) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Tahun Kurikulum (readonly dari relasi CPL) -->
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-800">
                                Tahun Kurikulum
                            </label>
                            <input type="text"
                                   value="{{ optional($id_cpl->tahun)->tahun ?? '-' }}"
                                   readonly
                                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-800 bg-gray-50">
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" id="kode_cpl" name="kode_cpl"
                                   ...
                                   value="{{ old('kode_cpl', $id_cpl->kode_cpl) }}"
                                   required>
                        </div>

                        <!-- Status CPL (opsional, sama seperti create) -->
                        <div class="space-y-2">
                            ...
                            <select id="status_cpl" name="status_cpl"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                <option value="" {{ old('status_cpl', $id_cpl->status_cpl) ? '' : 'selected' }}>Tidak ada status</option>
                                <option value="Kompetensi Utama Bidang" {{ old('status_cpl', $id_cpl->status_cpl) === 'Kompetensi Utama Bidang' ? 'selected' : '' }}>
                                    Kompetensi Utama Bidang
                                </option>
                                <option value="Kompetensi Tambahan" {{ old('status_cpl', $id_cpl->status_cpl) === 'Kompetensi Tambahan' ? 'selected' : '' }}>
                                    Kompetensi Tambahan
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="deskripsi_cpl" name="deskripsi_cpl" rows="4"
                                  class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                  required>{{ old('deskripsi_cpl', $id_cpl->deskripsi_cpl) }}</textarea>
                    </div>
