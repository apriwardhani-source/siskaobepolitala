...
@section('content')
    <div class="ml-20 mr-20">
        <h1 class="text-4xl font-extrabold mb-6 text-center">Edit Capaian Pembelajaran Mata Kuliah</h1>
        ...
        <form action="{{ route('tim.capaianpembelajaranmatakuliah.update', $cpmk->id_cpmk) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <input type="text" name="kode_cpmk" id="kode_cpmk" value="{{ old('kode_cpmk', $cpmk->kode_cpmk) }}"
            ...
            <input type="text" name="deskripsi_cpmk" id="deskripsi_cpmk"
                value="{{ old('deskripsi_cpmk', $cpmk->deskripsi_cpmk) }}"
                class="border border-black p-3 w-full rounded-lg mt-1 mb-3" required>

            {{-- Tombol --}}
            <button type="submit"
                class="px-5 py-2 bg-green-600 rounded-lg hover:bg-green-800 text-white font-bold mt-4">Simpan</button>
