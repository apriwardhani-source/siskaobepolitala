...
@section('content')
    <div class="bg-white p-4 md:p-6 lg:p-8 rounded-lg shadow-md mx-2 md:mx-0">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Data Visi</h1>
                    ...
                    <table class="w-full border border-black rounded-lg overflow-hidden shadow-md">
                        <thead class="bg-green-800 text-white">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-center font-medium uppercase border-r border-gray-200">Visi</th>
                        ...
                        <tbody>
                            @foreach ($Visis as $index => $visi)
                                <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-100">
                                    ...
                                    <td class="px-6 py-4 text-center border border-gray-200">{{ $visi->visi }}</td>
                                    <td class="px-6 py-4 text-center border border-gray-200">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('admin.visi.detail', $visi->id) }}"
                                            ...
                                            <a href="{{ route('admin.visi.edit', $visi->id) }}"
                                            ...
                                            <form action="{{ route('admin.visi.destroy', $visi->id) }}" method="POST"
                                                onsubmit="return confirm('Yakin ingin menghapus visi ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
