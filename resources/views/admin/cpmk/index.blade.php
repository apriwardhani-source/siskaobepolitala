                ...
                <div class="overflow-x-auto">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-white/10">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Kode CPMK</th>
                        ...
                        <tbody class="bg-white/10 divide-y divide-gray-200">
                            @forelse($cpmks as $cpmk)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ Str::limit($cpmk->deskripsi, 100) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $cpmk->mataKuliah->nama_matkul ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $cpmk->mataKuliah->prodi->nama_prodi ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            <!-- Tombol Edit -->
                                            <a href="{{ route('cpmk.edit', $cpmk->id) }}" class="text-blue-400 hover:text-blue-300">
                                            ...
                                            <form action="{{ route('cpmk.destroy', $cpmk->id) }}" method="POST" class="inline">
                                                ...
                                                <button type="submit" class="text-red-400 hover:text-red-300" 
                                                        onclick="return confirm('Yakin ingin menghapus CPMK {{ $cpmk->kode_cpmk }}?\nData yang dihapus tidak bisa dikembalikan.')">
