                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="flex items-end space-x-3">
                            ...
                            <a href="{{ route('wadir1.export.pemetaan', ['kode_prodi' => ($kode_prodi ?? request('kode_prodi')), 'id_tahun' => ($id_tahun ?? request('id_tahun'))]) }}"
                               class="inline-flex items-center px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow">
                ...
                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded mb-6">
                    <div class="flex">
                        ...
                        <div class="flex-1 text-sm text-yellow-800">
                            Tidak ditemukan data pemetaan untuk filter yang dipilih. Silakan coba kombinasi filter lain.
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($isFiltered && !empty($matrix))
                <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h2 class="text-lg font-semibold text-gray-900">Matriks Pemetaan MK - CPL - CPMK</h2>
                        @if(!empty($prodi))
                            <p class="mt-1 text-xs text-gray-500">
                                Program Studi: <span class="font-semibold">{{ $prodi->nama_prodi ?? '-' }}</span>
                                @if(!empty($id_tahun) && !empty($tahun_tersedia))
                                    @php
                                        $tahunLabel = optional($tahun_tersedia->firstWhere('id_tahun', $id_tahun))->tahun;
                                    @endphp
                                    · Tahun Kurikulum: <span class="font-semibold">{{ $tahunLabel ?? '-' }}</span>
                                @endif
                            </p>
                        @endif
                    </div>
