...
@section('title', 'Pemenuhan CPL (CPMK & MK) - Wadir 1')
@section('content')
  ...
  <div class="max-w-7xl mx-auto">
    <div class="mb-8">
      <div class="flex items-center space-x-4">
        ...
        <div>
          <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Pemenuhan CPL berdasarkan CPMK & MK</h1>
          <p class="mt-1 text-sm text-gray-600">Matriks ketercapaian CPL dari CPMK dan MK</p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 mb-8">
      ...
      <div class="p-6">
        <form method="GET" action="{{ route('wadir1.pemetaancplcpmkmk.pemenuhancplcpmkmk') }}" class="space-y-4">
    ...
    @if(!empty($matrix))
      <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
        <div class="px-6 py-4 border-b bg-gray-50"><h2 class="text-lg font-semibold text-gray-800">Matriks Pemenuhan</h2></div>
        <div class="p-6 space-y-4">
          @foreach($matrix as $kodeCpl => $row)
            <div class="border rounded-lg p-4">
              <div class="font-semibold text-gray-800">CPL {{ $kodeCpl }}</div>
              <div class="mt-2 grid md:grid-cols-2 gap-3">
                @foreach(($row['cpmk'] ?? []) as $kodeCpmk => $mks)
                  <div>
                    <div class="text-sm font-medium text-gray-700">CPMK {{ $kodeCpmk }}</div>
