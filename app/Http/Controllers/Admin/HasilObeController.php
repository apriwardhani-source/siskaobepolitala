...
class HasilObeController extends Controller
{
    ...
    public function detail($nim)
    {
        ...
        // Ambil semua nilai mahasiswa untuk mata kuliah yang diambil
        $nilaiMahasiswa = DB::table('nilai_mahasiswa')
            ->where('nim', $nim)
            ->get()
            ->keyBy('kode_mk');
        
        // Query untuk mengambil SEMUA CPMK per MK (tidak hanya yang ada di nilai)
        $hasil_obe = DB::table('mata_kuliahs as mk')
            ...
            ->map(function($item) use ($nilaiMahasiswa) {
                // Ambil nilai dari nilai_mahasiswa
