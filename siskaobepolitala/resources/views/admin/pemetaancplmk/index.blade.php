...
@section('title', 'Pemetaan CPL - MK - Admin')
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      ...
      <div class="px-6 py-5 border-b border-gray-200 bg-white">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <form action="{{ route('admin.pemetaancplmk.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end w-full">
            <div>
        ...
        <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div class="flex flex-wrap gap-2 items-center">
            ...
            @endif
            <a href="{{ route('admin.pemetaancplmk.index') }}" class="text-xs text-blue-600 hover:text-blue-800 hover:underline font-medium">Reset filter</a>
          </div>
        </div>
        @endif
      </div>
      ...
      <div class="px-6 py-4 bg-blue-50 border-b border-blue-100">
        <div class="flex items-start">
          ...
          <div class="text-sm text-blue-800">Centang menunjukkan CPL terpetakan ke MK. Tampilan ini bersifat read-only untuk Admin.</div>
        </div>
      </div>
      ...
      @elseif (($cpls ?? collect())->isEmpty())
        <div class="p-10 text-center">
          <div class="flex justify-center mb-4">
            <div class="w-20 h-20 rounded-2xl bg-yellow-500 text-white flex items-center justify-center shadow-lg">
              <i class="fas fa-exclamation-triangle text-3xl"></i>
            </div>
          </div>
          <h3 class="text-xl font-semibold text-gray-800">Belum Ada Data Pemetaan</h3>
          <p class="text-gray-600 mt-1">Data belum dibuat untuk program studi dan tahun yang dipilih.</p>
        </div>
      @else
