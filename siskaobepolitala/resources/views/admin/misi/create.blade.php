    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <div class="flex items-center space-x-4">
                <div class="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-emerald-500 text-white flex items-center justify-center shadow-lg">
                    <i class="fas fa-list-ul text-xl"></i>
                </div>
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight">Tambah Misi</h1>
                    <p class="mt-1 text-sm text-gray-600">
                        Tambahkan misi baru yang terkait dengan visi institusi.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
            ...
            <div class="px-6 py-6">
                ...
                <form action="{{ route('admin.misi.store') }}" method="POST" class="space-y-6">
                    @csrf

                    <div class="grid grid-cols-1 gap-6">
                        <div class="space-y-2">
                            <label for="visi_id" class="block text-sm font-semibold text-gray-800">Visi Terkait</label>
                            <select name="visi_id" id="visi_id" required
                                ...
                                <option value="" disabled {{ old('visi_id') ? '' : 'selected' }}>Pilih Visi</option>
                                @foreach ($visis as $visi)
                                    <option value="{{ $visi->id }}" {{ old('visi_id') == $visi->id ? 'selected' : '' }}>
                                        {{ $visi->visi }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-2">
                            <label for="misi" class="block text-sm font-semibold text-gray-800">Misi</label>
                            <textarea id="misi" name="misi" rows="4" required
                                      class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      placeholder="Tuliskan rumusan misi institusi secara ringkas dan jelas...">{{ old('misi') }}</textarea>
                        </div>
                    </div>
